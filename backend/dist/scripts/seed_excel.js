...
async function main() {
    ...
    if (!plan)
        throw new Error('No hay Plan Estrategico. Por favor corre el seed inicial primero.');
    ...
    let insertCount = 0;
    // Objetivos Map
    const objetivosCreados = new Map();
    ...
    const programasCreados = new Map();
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        if (stateVal === 'completado' || stateVal === 'completada')
            estadoInicial = 'COMPLETA_SIN_VALIDAR';
        else if (stateVal === 'en curso')
            estadoInicial = 'EN_CURSO_SIN_VALIDAR';
