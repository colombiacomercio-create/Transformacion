...
router.get('/periodo/:date', auth_middleware_1.azureADAuth, async (req, res) => {
    try {
...
router.get('/ultima', auth_middleware_1.azureADAuth, async (req, res) => {
    try {
        const ficha = await prisma.fichaResultados.findFirst({
            include: incluyeReportadores,
            orderBy: { periodo: 'desc' },
    ...
    try {
        const ficha = await prisma.fichaResultados.update({
            ...
            data: {
                ...
                ...Object.fromEntries(Object.entries(data).filter(([k]) => k !== 'periodo' && k !== 'seccionesActualizadas').map(([k, v]) => [k, v ?? null])),
            },
        });
        res.json(ficha);
    }
