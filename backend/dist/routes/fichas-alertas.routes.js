...
router.patch('/:id/estado', auth_middleware_1.azureADAuth, async (req, res) => {
    ...
    try {
        ...
        if (req.user.rol === 'OBSERVADOR')
            return res.status(403).json({ error: 'No autorizado para cambiar estado' });
...
router.post('/:id/actualizaciones', auth_middleware_1.azureADAuth, async (req, res) => {
    ...
    try {
        ...
        if (req.user.rol === 'OBSERVADOR') {
            ...
            const correosAutorizados = (alerta.correosResponsables || []).map((e) => e.toLowerCase());
