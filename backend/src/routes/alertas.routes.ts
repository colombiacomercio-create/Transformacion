...
router.get('/', azureADAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (user.rol === 'GESTOR') {
      ...
      validFilter = { localidadId: { in: localidadesGestor.map((l: any) => l.id) } };
    }
...
router.post('/ejecutar-reglas', azureADAuth, requireRole(['ADMIN']), async (req, res) => {
  try {
      ...
      for (const asig of param.act.asignaciones) {
        ...
        if (asig.responsable && asig.responsable.email) {
          await enviarNotificacion(
            [asig.responsable.email],
            `Alerta Sistema Transformación: ${param.tipo}`,
            `<h3>Atención</h3><p>La actividad <b>${param.act.nombre}</b> requiere su acción urgente.</p>`
          );
        }
      }
    }
