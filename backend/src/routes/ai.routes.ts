...
router.post('/reportes/generar-borrador', azureADAuth, requireRole(['ADMIN']), async (req: AuthRequest, res) => {
  try {
    ...
    const rezagadas = asignaciones
      .filter(a => {
        if (!a.actividad.fechaLimite) return false;
    ...
    if (latestFichaRes) {
      ...
      if (objCodigo === 'O1') alertaFichaResultados = latestFichaRes.alertaEjecucion || "";
      else if (objCodigo === 'O2') alertaFichaResultados = latestFichaRes.alertaObras || "";
      else if (objCodigo === 'O3') alertaFichaResultados = `${latestFichaRes.alertaEspacioResiduos || ""} ${latestFichaRes.alertaEspacioVenta || ""}`.trim();
      else if (objCodigo === 'O4') alertaFichaResultados = latestFichaRes.alertaConvivencia || "";
      else if (objCodigo === 'O5') alertaFichaResultados = latestFichaRes.alertaRollos || "";
      else if (objCodigo === 'OV1') alertaFichaResultados = latestFichaRes.alertaEstrategias || "";
      else if (objCodigo === 'O6') alertaFichaResultados = latestFichaRes.alertaActuaciones || "";
    }
  ...
  try {
    ...
    if (chatResponse.parametrosFiltro && Object.keys(chatResponse.parametrosFiltro).length > 0) {
      ...
      if (esConsultaVencidas) {
        ...
        const ahora = new Date();
        const vencidas = allAsig.filter((a: any) => {
          if (!a.actividad.fechaLimite) return false;
          const limite = new Date(a.actividad.fechaLimite);
          return limite < ahora && a.porcentajeAvance < 100 && a.estadoLocal !== 'COMPLETA_SIN_VALIDAR';
        });
