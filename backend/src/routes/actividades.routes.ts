...
router.get('/', azureADAuth, async (req: AuthRequest, res) => {
  try {
    ...
    let filtroLocalidad = {};
    let validLocalidadIds: string[] | undefined = undefined;
    ...
    if (user.rol === 'GESTOR') {
      ...
      if (asignaciones.length === 0 && user.id === 'gestor-local-123') {
         ...
         if (locPiloto) asignaciones = await prisma.asignacionLocalidad.findMany({ where: { localidadId: locPiloto.id }});
      }
...
router.post('/', azureADAuth, requireRole(['ADMIN', 'GESTOR']), async (req: AuthRequest, res) => {
  try {
...
// Agregar Comentario
router.post('/:id/comentarios', azureADAuth, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    ...
    if (!locIdFinal) return res.status(400).json({ error: 'No data localidad' });
...
router.patch('/subtarea/:id', azureADAuth, requireRole(['ADMIN', 'GESTOR']), async (req: AuthRequest, res) => {
  try {
...
router.patch('/asignacion/:id/estadoLocal', azureADAuth, requireRole(['ADMIN', 'GESTOR']), async (req: AuthRequest, res) => {
  try {
...
router.patch('/asignacion/:id/estadoValidacion', azureADAuth, requireRole(['ADMIN']), async (req: AuthRequest, res) => {
  try {
